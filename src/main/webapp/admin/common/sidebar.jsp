...
<nav class="admin-sidebar">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // Chọn tất cả các link có class sidebar-link nằm trong thẻ li có class menu-item-has-children
            var dropdowns = document.querySelectorAll(".menu-item-has-children > .sidebar-link");

            dropdowns.forEach(function (link) {
                link.addEventListener("click", function (e) {
                    e.preventDefault(); // Ngăn chặn chuyển trang khi click vào menu cha
                    var parent = this.parentElement;

                    // Toggle class 'open' để đóng mở submenu
                    parent.classList.toggle("open");
                });
            });

            // Logic giữ trạng thái active cho menu cha nếu đang ở trang con
            var currentUrl = window.location.href;
            var subLinks = document.querySelectorAll(".submenu a");

            subLinks.forEach(function(link) {
                if(currentUrl.includes(link.getAttribute("href"))) {
                    // Nếu link con trùng với URL hiện tại, thêm class active cho cha nó
